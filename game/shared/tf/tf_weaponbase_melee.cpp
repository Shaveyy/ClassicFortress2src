...
void CTFWeaponBaseMelee::Smack( void )
{
	...
	if ( DoSwingTrace( trace ) )
	{
		...
		float flDamage = 18.0f; // melee damage for crowbar
...
CBaseEntity* CTFWeaponBaseMelee::FireProjectile(CTFPlayer* pPlayer)
{
...
//
	...
	case TF_PROJECTILE_PIPEBOMB:
		pProjectile = FirePipeBomb(pPlayer, false,300);
		pPlayer->DoAnimationEvent(PLAYERANIMEVENT_ATTACK_PRIMARY);
		break;

	case TF_PROJECTILE_PIPEBOMB_REMOTE:
		pProjectile = FirePipeBomb(pPlayer, true,300);
		pPlayer->DoAnimationEvent(PLAYERANIMEVENT_ATTACK_PRIMARY);
		break;
	...
	default:
		// do nothing!
		pProjectile = FirePipeBomb(pPlayer, false,300);
		DevMsg("Weapon has no projectile, so we chose PipeBomb for you\n");
		break;
	}
...
CBaseEntity* CTFWeaponBaseMelee::FirePipeBomb(CTFPlayer* pPlayer, bool bRemoteDetonate,float range)
{
	...
	Vector vecVelocity = (vecForward * range) + (vecUp * 200.0f) + (random->RandomFloat(-10.0f, 10.0f) * vecRight) +
...
void CTFWeaponBaseMelee::UpdatePunchAngles(CTFPlayer* pPlayer)
{
...
float CTFWeaponBaseMelee::GetProjectileSpeed(void)
{
	// placeholder for now
