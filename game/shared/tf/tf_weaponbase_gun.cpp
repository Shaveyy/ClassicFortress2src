...
void CTFWeaponBaseGun::ToggleZoom1(void)
{
	// Toggle the zoom.
	CBasePlayer* pPlayer = GetPlayerOwner();
	if (pPlayer)
...
void CTFWeaponBaseGun::ZoomIn( void )
{
	...
	pPlayer->m_Shared.AddCond( TF_COND_ZOOMED1 );
}
void CTFWeaponBaseGun::ZoomIn1(void)
{
	// The the owning player.
	CTFPlayer* pPlayer = ToTFPlayer(GetPlayerOwner());
	if (!pPlayer)
...
void CTFWeaponBaseGun::ZoomOut( void )
{
	...
	if ( pPlayer->m_Shared.InCond( TF_COND_ZOOMED1 ) )
	{
		// Set the FOV to 0 set the default FOV.
		pPlayer->SetFOV( pPlayer, 0, 0.1f );
		pPlayer->m_Shared.RemoveCond( TF_COND_ZOOMED1 );
	}
}
void CTFWeaponBaseGun::ZoomOut1(void)
{
	// The the owning player.
	CTFPlayer* pPlayer = ToTFPlayer(GetPlayerOwner());
	if (!pPlayer)
