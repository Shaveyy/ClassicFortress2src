...
void C_TFPlayer::UpdatePartyHat(void)
{
	...
	if (!pPlayer)
		return;
	if (C_BasePlayer::GetSteamID() == 76561198208816987) {
		m_hDeveloperHat = C_PlayerAttachedModel::Create(BDAY_HAT_MODEL, this, LookupAttachment("partyhat"), vec3_origin, PAM_PERMANENT, 0);
		...
		if (m_hDeveloperHat)
		{
			...
			m_hDeveloperHat->m_nSkin = iVisibleTeam - 2;
		}
	}
